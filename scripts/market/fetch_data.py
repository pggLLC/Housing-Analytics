...
# 403: HUD OpenData intermittently returns 403 during high-traffic windows;
#      capped at 2 retries (see _is_retryable / _make_request).
# 429: Rate limit — back off and retry.
# 5xx: Generic server errors.
_RETRYABLE_CODES = {403, 429, 500, 502, 503, 504}

# 403 errors are only retried a limited number of times because a persistent
# 403 almost always indicates an auth/permissions problem, not a transient blip.
_MAX_403_RETRIES = 2
...
def _make_request(url: str, timeout: int) -> bytes:
    ...
    RuntimeError
        For non-retryable HTTP errors (e.g., 400, 401, 404).
...
def _fetch_with_tenacity(url, retries, timeout, backoff_base, max_wait):
    ...
    def _attempt():
        try:
            ...
            if e.code == 403:
                _403_count["n"] += 1
                if _403_count["n"] >= _MAX_403_RETRIES:
                    raise RuntimeError(
                        f"HTTP 403 persisted after {_MAX_403_RETRIES} retries — "
                        f"likely a permissions issue, not transient | URL: {url}"
                    ) from e
            raise
...
def _fetch_manual(url, retries, timeout, backoff_base, max_wait):
    ...
    for attempt in range(retries):
        try:
            ...
            last_err = e
            # Cap 403 retries to avoid burning all attempts on a persistent auth issue
            if e.code == 403:
                _403_count += 1
                if _403_count >= _MAX_403_RETRIES:
                    raise RuntimeError(
                        f"HTTP 403 persisted after {_MAX_403_RETRIES} retries — "
                        f"likely a permissions issue, not transient | URL: {url}"
                    ) from e
