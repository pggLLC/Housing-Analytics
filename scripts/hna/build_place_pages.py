...
{{PLACE_JSON_LD}}
  ...
  <style>
    ...
    .place-grid { max-width: var(--page-max, 1920px); margin: 1.5rem auto; padding: 0 1.5rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(min(280px, 100%), 1fr)); gap: 1rem; }
    ...
    .place-disclosure { margin: 1rem auto; max-width: var(--page-max, 1920px); padding: .75rem 1rem; background: rgba(59, 130, 246, .08); border: 1px solid rgba(59, 130, 246, .3); border-radius: 6px; font-size: .88rem; line-height: 1.5; }
    .place-tools { max-width: var(--page-max, 1920px); margin: 2rem auto; padding: 0 1.5rem; }
    ...
    .place-source { max-width: var(--page-max, 1920px); margin: 2rem auto; padding: 0 1.5rem; font-size: .78rem; color: var(--muted); }
  </style>
</head>
<body>
  ...
  <main id="main-content">
    <div class="place-hero">
      <div style="max-width: var(--page-max, 1920px); margin: 0 auto;">
    ...
    (function () {
      ...
      (function () {
        ...
        if (r != null) {
          ...
          var needExplain = pvn.need_method === 'place_permit_blend'
            ? 'Need is this community\\'s own projection — the county DOLA forecast downscaled by a 50/50 blend of its household share and its share of county building permits' +
              (pvn.blended_share_used != null ? ' (blended share ' + Math.round(pvn.blended_share_used * 100) + '%)' : '') +
              ' — the same figure the HNA dashboard shows'
            : 'Need is the county DOLA projection scaled to this community\\'s share of county households (' +
              Math.round((pvn.county_share_used || 0) * 100) + '%)';
          say('At the recent pace (' + one(avg.value) + ' permitted units/yr, ' + (avg.window || '') + '), production ' + closes +
              ...
              '. ' + needExplain + '; it excludes existing shortfalls, so treat it as a floor.');
        } else {
...
<head>
  ...
  <script defer src="../js/navigation.js"></script>
  <style>
    .place-index {{ max-width: var(--page-max, 1920px); margin: 2rem auto; padding: 0 1.5rem; }}
