...
# internal/docs/codex-audits/HANDOFF-PROMPTS.md — keep them in sync.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PACKAGES_DIR="$ROOT/internal/docs/codex-audits"
...
oneliner() {
  local g="$1"
  cat <<EOF
Open internal/docs/codex-audits/$g.md and complete the source-first audit
...
Per-brief audit packages have been pre-built at internal/docs/codex-audits/.
...
internal/docs/codex-audits/$g.md and follow that file step by step. Use
...
under internal/docs/codex-audits/ (excluding README.md and HANDOFF-PROMPTS.md)
