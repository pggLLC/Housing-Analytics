...
def main():
    ...
    for geo in all_geos:
        ...
        # 3. has_housing_authority
        if lr:
            ha = lr.get('housingAuthority', [])
            dims['has_housing_authority'] = bool(ha and len(ha) > 0)
        else:
            dims['has_housing_authority'] = None

        # 4. has_housing_nonprofits
        if lr:
            adv = lr.get('advocacy', [])
            dims['has_housing_nonprofits'] = bool(adv and len(adv) > 0)
        else:
