...
1. **Keep the configured developer gate.** It is *intentional* UI gating for `/developer*` pages pending Cloudflare Access (see `js/developer-gate.js`, `docs/codex-audits/PASTE-INTO-CODEX.md`). Do **not** delete the gate or its hash. You may remove the *plaintext password from comments/docs* — that is the only change allowed here.
