...
Inventory (derived — run `node scripts/compute-inventory.mjs`): **54** top-level / **548** total HTML · **67** workflows · **267** client JS (**262** excl. `vendor/`) · **240** build/fetch scripts · **497** `docs/` markdown · **1648** data files · **547** geo-config rows / **546** unique geographies (**64** counties / **273** places / **210** CDPs) · **926** CHFA LIHTC features.

Notes on numbers that look like contradictions but aren't:
- **547 vs 546.** `data/hna/geo-config.json` has 547 rows but 546 distinct GEOIDs — GEOID `0812900` (Central City, Gilpin) appears twice, at `places[41]` ("Central (city)") and `places[42]` ("Central City (city)"). `ranking-index.json` therefore carries **546** entries, correctly. Both numbers are right at different scopes. Known bug, tracked separately — do not "fix" it as a drive-by, it moves counts in ~10 files plus two audit thresholds.
