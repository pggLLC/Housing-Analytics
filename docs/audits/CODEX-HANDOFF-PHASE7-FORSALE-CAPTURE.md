...
2. **Gross contracts needed** — the funnel already applies `contract_fallout` (net). **ERRATUM (2026-08-05, found in PR #1404 QA):** the funnel's `contract_fallout` stage share is a **survival multiplier** (share 0.85 ⇒ 85% of contracts close), so gross contracts = **closings ÷ share** — NOT ÷(1 − share) as this section originally read (that formula treated the survival share as a fallout rate and yielded absurd results: 333 gross contracts for 50 closings at 15% fallout; correct: 58.8). The share is **read from the funnelResult's `contract_fallout` stage** — never a second input (double-count guard: a test asserts the module has no fallout parameter of its own). If the funnel's fallout stage is unresolved, gross contracts are `not_available` even when net closings are computable.
