...
**Blocker to fix first**: `js/deal-calculator.js` has no `module.exports` — a test can't `require()` it directly today (same gap #1152 has for `market-analysis.js`).

**Implementation**:
1. Add a `module.exports` block to `js/deal-calculator.js` exposing at minimum `mortgageConstant` (the pure, dependency-free function most worth testing directly). Follow the existing pattern already used in `js/pma-competitive-set.js` (which the current `test/pma-competitive-set.test.js` already `require()`s successfully) — check how that file structures its exports section and mirror it, rather than inventing a new convention.
2. Add a new test file (e.g. `test/deal-calc-mortgage-math.test.js`) asserting `mortgageConstant()` and the full NOI/DCR → mortgage calculation against 2-3 hand-computed fixtures. Suggested fixtures (all independently verified, safe to use as-is):
...
**Tests**: the new test file itself is the deliverable. Also run the full `test:ci` suite to confirm the new `module.exports` doesn't break anything that currently relies on `js/deal-calculator.js` being a non-module script (check how it's loaded in `deal-calculator.html` — a `<script>` tag expects browser-global behavior; `module.exports` guarded by `typeof module !== 'undefined'` is the standard safe pattern used elsewhere in this repo for exactly this dual-context need, e.g. check `js/pma-competitive-set.js`'s own guard for the exact idiom).
...
**Implementation**:
1. Add a `module.exports` block to `js/market-analysis.js` for the pure-function scoring helpers: at minimum `scoreCaptureRisk`, `scoreMarketTightness`, `scoreRentPressure`, and any other `score*` function `pma-scoring.test.js`/`pma-confidence.test.js` currently reimplements. Same dual-context guard pattern as item 1.
2. Rewrite `test/pma-scoring.test.js` (and `pma-confidence.test.js` if it has the same issue — check) to `require('../js/market-analysis.js')` and call the real functions instead of the inline copies. Delete the inline reimplementations once the real ones are wired in — don't leave both.
3. Add a dedicated test for `scoreCaptureRisk`'s CHAS-branch behavior: (a) with `chasEligible = {value: N, ...}` populated, confirm the function uses `N` as the denominator, not `acs.renter_hh`; (b) with `chasEligible` absent/null, confirm it falls back to `acs.renter_hh`, matching current documented behavior.

**Tests**: the rewritten test file is the deliverable, plus the new CHAS-branch test in item 3 above.
...
**What's wrong**: two different, differently-defined formulas are both labeled "capture rate" in the live UI. `scoreCaptureRisk()` (`js/market-analysis.js:726-746`) computes a demand-pool ratio (units ÷ CHAS-eligible-or-ACS renter households) rendered into the permanent stat tile `#pmaCaptureRate` (`market-analysis.html:744`). `calculateAbsorptionRisk()` (`js/pma-competitive-set.js:276-297`) computes a competitive-supply-share ratio (proposed units ÷ existing competitive supply), rendered as narrative text at `js/pma-ui-controller.js:544`, also labeled "capture rate." A user can see both, differently defined, on the same page.
...
2. `docs/PMA_SCORING.md` — currently documents `scoreCaptureRisk`'s formula and `RISK.captureHigh = 0.25` threshold but says nothing about `calculateAbsorptionRisk()`'s `SATURATION_LIMIT = 0.10`. Add a short section documenting the competitive-supply-share formula and its threshold, using whatever label was chosen in step 1.
