...
- **Ownership Fit** from two terms:
  1. `moderateIncomeRenterHouseholds` (count and share of renters) — the *base-exists* term. The caveat text must explicitly disclaim purchase readiness (e.g. "not evidence of purchase readiness") — phrased WITHOUT using the banned terms themselves, so the copy passes the banned-phrase test.
...
3. **Potential Shared-Equity Fit** — moderate-income (51–100% HAMFI) renter households; tier; caveat: "These counts are not evidence of purchase readiness — they only screen for whether an ownership-oriented base may exist." (Deliberately avoids the banned "qualified buyer(s)" phrasing so UI copy passes the banned-phrase test.)
