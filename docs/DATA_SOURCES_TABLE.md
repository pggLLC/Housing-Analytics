...
|------|---------|----------------------|-------------|-----------------|-------------------|--------|
| `data/chfa-lihtc.json` | **0 (empty — CI not yet run)** | CHFA ArcGIS FeatureServer (public) | `fetch-chfa-lihtc.yml` (Mon 05:00 UTC) + `deploy.yml` on every push | None — public API | 1. Local file → 2. CHFA ArcGIS → 3. HUD ArcGIS → 4. 14 embedded projects | ⚠️ Empty — triggers remote API fallback on page load |
...
- The `data/chfa-lihtc.json` file is currently empty because the CI workflow has not yet run after the latest branch push.  When a user loads the Colorado Deep Dive map, the front-end automatically falls back to the CHFA ArcGIS API (15 s timeout), then HUD ArcGIS, and finally the 14 embedded sample projects.
...
|-----------|------------------------|--------------------|
| `data/chfa-lihtc.json` | ⚠️ **Partially** — created by CI, currently empty | Script is correct but CI hasn't populated it yet. Front-end automatically falls back to live CHFA/HUD ArcGIS APIs and then 14 embedded projects. The `fetch-chfa-lihtc.js` now preserves a non-empty existing file if the live API returns 0 features. |
