...
- **CAR real-data ingest (Gate 0 — owner licensing decision).** Explored 2026-07-01: real CAR market data IS available + machine-readable (ShowingTime `sst` HTML, `marketstatsreports.showingtime.com/CAR-Colorado_hqac0/sst/<YYYYMM>/{0SF,0TC}.htm` — all 64 counties + CDs; newest = May 2026, ~1-month lag). BUT two blockers: (1) **licensing** — it's MLS data (© ShowingTime, "provided by CAR"); republishing on the public site needs CAR permission + attribution; (2) **access** — the host is WAF/bot-protected (CI fetch unreliable). Field coverage is only 4/8 schema fields (median price, closed, new listings, active); DOM/months-supply are statewide-only in the press release; $/sqft + list-to-sale unavailable. Metros must be aggregated from member counties. Full ingest spec (ShowingTime SF+TC, county→metro map, 0/low-sample handling, 64-county May-2026 seed data, placeholder fallback) was worked out in chat 2026-07-01 — formalize into a task ONLY after the owner clears Gate 0. Interim: keep `generate-car-placeholder.mjs` (trend-projected placeholders, labeled "estimated"), or cite press-release headline figures with attribution, or repoint to the real in-repo Zillow metro CSVs (different metrics).
