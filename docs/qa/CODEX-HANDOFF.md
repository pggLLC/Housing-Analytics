...
**Cleanup — the ONLY Codex task here (one small DRAFT PR):**
- **C-0a — `_manifest.json` leaked a non-deployed file:** it lists `zillow/city_zhvi_…csv` (92.9 MB), so `js/data-explorer.js` renders a phantom `zillow/` entry that 404s on the live site. The `data/_manifest.json` generator (`scripts/audit/build-data-manifest.mjs` — feeds data-explorer) must EXCLUDE the non-deployed `data/zillow/*.csv` files; then regenerate `_manifest.json` clean. **QA 2026-06-30 (curl):** ALL `data/zillow/*.csv` 404 on the live site — the untracked 92.9 MB city CSV AND the 3 *tracked* metro CSVs (`zhvi/zori/median_list_price_metro.csv`). So excluding the whole `data/zillow/*.csv` pattern is CORRECT (all phantom); do NOT special-case the tracked metro CSVs back in. **⚠️ Attempt #1009 implemented the exclusion correctly but on a STALE base (≈40 commits behind, predates #1008 — no `homeValueInfo`/stamper/hook); it must be REBASED onto current main and `_manifest.json` regenerated from current data before merge, or it will conflict with / revert #1008.**
...
**Open PRs — owner actions, NOT Codex tasks (all independent of the cleanup; no ordering):**
- **#1007** (Dependabot dev-deps: playwright/puppeteer/stylelint) — KEEP. Only blocker (properties-manifest `5a2256433186` vs `270188427a69`) is already fixed on main by #1008. Comment `@dependabot rebase` on the PR; merge when CI is green.
- **#1005** (auto-generated by `docs-sync.yml`; 83 `docs/` files — **docs-only**, does NOT touch `data/_manifest.json`) — CLOSE; re-run `docs-sync` (Actions → Run workflow; it has `workflow_dispatch`) for a fresh PR from current main. Don't hand-resolve the conflict.
- **#1001** (auto-generated source-health snapshot, 2026-06-28) — CLOSE; re-run `source-liveness-weekly` (`workflow_dispatch`) or wait for its Sunday cron.
