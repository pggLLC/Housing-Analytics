...
- **C-0a — `_manifest.json` leaked a gitignored file:** it now lists `zillow/city_zhvi_…csv` (92.9 MB, gitignored via `data/zillow/*.csv`), so `js/data-explorer.js` renders a phantom `zillow/` folder + a 92.9 MB entry that 404s on the live site. The `data/_manifest.json` generator — `scripts/audit/build-data-manifest.mjs` (header says it feeds data-explorer); also check `scripts/audit/data-inventory.mjs`, which `walk()`s `.csv` — must EXCLUDE gitignored paths (respect `.gitignore` / skip `data/zillow/*.csv`); then regenerate `_manifest.json` clean.
