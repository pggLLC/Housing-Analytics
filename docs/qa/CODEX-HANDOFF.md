...
1. **PIPELINE GUARD — THE fix (without it, re-breaks every daily run):** re-stamp must run as the LAST step of the data-refresh pipeline AND/OR make `build_ranking_index.py` read `median_home_value` from `home-value-cascade.json` DIRECTLY (DP04_0089E labeled `acs_raw` fallback; `missing` when neither) — so a summary rebuild can never silently revert it. Workflow-file edits are owner-gated — **FLAG** the `.github/workflows/*` change in the PR body, do NOT edit the workflow.
