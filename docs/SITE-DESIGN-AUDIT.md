...
> *Auto-synced 2026-03-16 by `scripts/sync-docs.mjs` · 31 pages · 840 data files · 29 workflows*
<!-- sync-banner:end -->

# COHO Analytics — Site Design & Functionality Audit

**Prepared:** March 2026 — *Updated March 2026 to reflect fixes*  
...
~~When a chart's data is still being fetched (or fails to load), the `<canvas>` element renders blank.~~
...
~~Several pages render `<table>` elements that overflow horizontally on screens narrower than 768px.~~
...
~~The PMA analysis pipeline has 9 steps. The progress bar (`#pmaProgressFill`) exists but its `aria-valuenow` attribute is not updated dynamically.~~
...
**6.1 — No resource hints for critical assets** ✅ **Fixed**  
~~The pages load `Chart.js`, `Leaflet.js`, and Google Fonts without `<link rel="preconnect">` or `<link rel="preload">` hints.~~

`<link rel="preconnect" href="https://fonts.googleapis.com">` and `<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>` are now present in `index.html` and key page templates.

**6.2 — Large JSON files are loaded synchronously** ✅ **Partially Fixed**  
...
`js/co-ami-gap.js` and `js/main.js` now use `IntersectionObserver` to defer loading below-the-fold charts. **Still outstanding:** `data/fred-data.json` is still loaded eagerly on `economic-dashboard.html`.

**Recommendation:** Wrap the FRED data fetch in an `IntersectionObserver` on the first FRED chart container.

**6.3 — Missing `<meta name="theme-color">` tag** ✅ **Fixed**  
~~Pages do not have a theme-color meta tag, so mobile browser chrome does not adopt the brand color.~~

Both light- and dark-mode `theme-color` meta tags are now present in `index.html`:
```html
...
**7.2 — Policy briefs and insights lack search/filter** ✅ **Partially Fixed**  
...
`policy-briefs.html` now includes a client-side topic-based filter. **Still outstanding:** `insights.html` does not yet have a filter bar.

**Recommendation:** Apply the same topic-filter pattern from `policy-briefs.html` to `insights.html`.
...
|----------|----------------|--------|--------|--------|
| 🔴 High | 5.5 — Keyboard navigation in geography selector | Medium | Accessibility / Legal | ⬜ Open |
| 🔴 High | 3.1 — Chart loading and empty states | Low | UX trust | 🔶 CSS done, JS pending |
| 🔴 High | 4.2 — Table overflow on mobile | Low | Mobile UX | ✅ Fixed |
| 🟠 Medium | 1.1 — User journey-based navigation | High | Discoverability | ⬜ Open |
| 🟠 Medium | 1.2 — Orphaned pages | Low | Discoverability | 🔶 2 of 3 fixed |
| 🟠 Medium | 1.3 — Breadcrumbs on analysis pages | Low | Navigation | 🔶 Partial |
| 🟠 Medium | 2.1 — Progressive disclosure on HNA page | Medium | Cognitive load | ⬜ Open |
| 🟠 Medium | 3.4 — Synchronized map and chart | Medium | Analytical UX | ⬜ Open |
| 🟠 Medium | 5.1 — Export feedback toasts | Low | Interaction quality | ⬜ Open |
| 🟡 Low | 2.3 — Color coding consistency | Low | Brand coherence | ⬜ Open |
| 🟡 Low | 6.1 — Resource hints | Low | Performance | ✅ Fixed |
| 🟡 Low | 6.2 — Lazy loading large JSON | Low | Performance | 🔶 Partial |
| 🟡 Low | 6.3 — theme-color meta tag | Low | Mobile UX | ✅ Fixed |
| 🟡 Low | 7.2 — Policy brief tag filter | Low | Content usability | 🔶 Partial |
| 🟡 Low | 7.3 — Glossary and acronym tooltips | Medium | Content usability | ⬜ Open |
| ✅ Done | 4.2 — Table overflow | — | — | ✅ Fixed |
| ✅ Done | 5.3 — PMA progress bar aria-valuenow | — | Accessibility | ✅ Fixed |
