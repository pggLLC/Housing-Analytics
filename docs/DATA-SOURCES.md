...
> **📋 Note:** For the authoritative live data catalog and GIS reliability audit, see [`SITE_AUDIT_GIS.md`](SITE_AUDIT_GIS.md). This file documents operational API secrets, CI workflow schedules, and data file inventory.
...
|---|---|---|---|
| `deploy.yml` | Push to main + manual | `CENSUS_API_KEY`, `FRED_API_KEY` | `js/config.js` (secrets injected into committed placeholder); also runs `fetch-chfa-lihtc.js` |
...
1. **`data/chfa-lihtc.json` feature count** — The CHFA ArcGIS FeatureServer now returns 716 features (fixed as of 2026-03-06). If the file becomes empty again, the front-end correctly falls back to the HUD ArcGIS API, then to embedded data (14 projects).
   **Action:** Manually run the `Fetch CHFA LIHTC Data` workflow from the Actions tab to force a refresh.
