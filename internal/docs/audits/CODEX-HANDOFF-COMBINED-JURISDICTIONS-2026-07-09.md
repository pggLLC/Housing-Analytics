...
**Data source to wire in**: `data/hna/home-value-cascade.json`. Schema (from its own `meta.schema`): `{ meta, places: { <7-digit place GEOID>: { value, source, as_of, confidence, ... } }, review_flags }` — note the `places` key needs unwrapping, the file isn't a flat GEOID map at the top level. This is place/CDP-level only — it has no county entries, so county members in a combo will naturally have no match and get filtered out by `weightedMetricRange`'s existing `value == null` skip (that's correct, expected behavior, not a bug to fix).

**Second, smaller gap in the same data**: of the 482 place entries in `places`, only **429** have `.value` actually populated — the other 53 have `value: null, confidence: "missing"` (verified by direct count; `meta.counts.total` is 482, which is total entries, not entries-with-value — don't cite that field as evidence of full coverage). None of those 53 have the `median_home_value` fallback field populated either, so `weightedMetricRange` will correctly skip them via its existing `value == null` filter — same graceful-degradation behavior as the county case above, not a new bug to fix. But it means a combo can be all-place and still land on `available: false` (or a smaller-than-expected member count in the range) if it happens to include one of those 53 — worth a one-line mention in the "not available" message's wording (or just accept the generic message) so a future reader doesn't mistake it for the fix not working.
...
**QA gate**: build a combo of 2 real places with known ZHVI entries (check `data/hna/home-value-cascade.json`'s `places` object for two GEOIDs with `source: "zhvi"` to pick real examples), confirm the rendered home-value stat shows an actual range/average instead of the literal text `"Range / modeled average"`, and confirm an all-county combo still shows "Not available" rather than throwing or showing a bogus value. Also build a combo including one of the 53 `confidence: "missing"` places (grep the file for `"confidence": "missing"` to find current examples) alongside a place that does have data, and confirm it degrades gracefully — the member with missing data is silently excluded from the range/average, not shown as `$0` or causing an error.
