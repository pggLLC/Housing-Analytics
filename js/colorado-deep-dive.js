...
(function () {
   ...
   *
   * Renders the "Mortgage Credit Access" section under Market Conditions.
   * Pulls two source files generated by scripts/fetch_hmda.py:
  ...
  function loadHmdaKpis() {
    ...
    var stateUrl    = 'data/hmda/co-state-trends.json';
    var countiesUrl = 'data/hmda/co-county-aggregates.json';

    Promise.all([DataService.getJSON(stateUrl), DataService.getJSON(countiesUrl)])
      .then(function (results) {
        var state = results[0];
        var counties = results[1];
        renderHmda(state, counties);
      ...
      .catch(function (err) {
        ...
        if (errorEl) {
          ...
          errorEl.textContent = 'HMDA data unavailable: ' + (err && err.message ? err.message : err);
        }
      });
  }

  function renderHmda(state, counties) {
    if (!state || !state.years) return;
    var years = Object.keys(state.years).sort();
    if (!years.length) return;
    var latest = years[years.length - 1];
    var prior  = years.length >= 2 ? years[years.length - 2] : null;
    var L = state.years[latest];
    var P = prior ? state.years[prior] : null;
    ...
    // YoY delta strings
    if (P) {
      setText('hmdaOriginationsDelta',  yoyDelta(L.originations, P.originations, prior));
      setText('hmdaDenialRateDelta',    yoyDeltaPct(L.denial_rate, P.denial_rate, prior));
      setText('hmdaMeanLoanDelta',      yoyDeltaCurrency(L.mean_loan_amount_usd, P.mean_loan_amount_usd, prior));
      setText('hmdaMultifamilyDelta',   yoyDelta(L.multifamily.originations, P.multifamily.originations, prior));
    }

    // Freshness label
    var freshness = document.getElementById('hmdaFreshness');
    if (freshness) {
      freshness.textContent = ' Latest: ' + latest + ' (vintage ' + years[0] + '–' + latest + ')';
    }

    // Top-10 counties table
    if (counties && counties.counties) {
      var rows = Object.values(counties.counties)
        .map(function (c) {
          var y = c.years && c.years[latest];
          return y ? {
            name: c.name,
            fips: c.fips,
            origs: y.originations,
            volume: y.origination_volume_usd,
            denial: y.denial_rate,
            mf: y.multifamily ? y.multifamily.originations : 0
          } : null;
        })
        .filter(Boolean)
        .sort(function (a, b) { return b.origs - a.origs; })
        .slice(0, 10);

      var html = '<table style="width:100%;border-collapse:collapse;font-size:.88rem;">' +
        '<thead><tr>' +
          '<th style="text-align:left;padding:.4rem .6rem;border-bottom:1px solid var(--border,#333);">County</th>' +
          '<th style="text-align:right;padding:.4rem .6rem;border-bottom:1px solid var(--border,#333);">Originations</th>' +
          '<th style="text-align:right;padding:.4rem .6rem;border-bottom:1px solid var(--border,#333);">Volume ($M)</th>' +
          '<th style="text-align:right;padding:.4rem .6rem;border-bottom:1px solid var(--border,#333);">Denial Rate</th>' +
          '<th style="text-align:right;padding:.4rem .6rem;border-bottom:1px solid var(--border,#333);">MF Loans</th>' +
        '</tr></thead><tbody>';
      rows.forEach(function (r) {
        html += '<tr>' +
          '<td style="padding:.35rem .6rem;border-bottom:1px solid var(--border-soft,#222);">' + escapeHtml(r.name) + '</td>' +
          '<td style="text-align:right;padding:.35rem .6rem;border-bottom:1px solid var(--border-soft,#222);font-variant-numeric:tabular-nums;">' + r.origs.toLocaleString() + '</td>' +
          '<td style="text-align:right;padding:.35rem .6rem;border-bottom:1px solid var(--border-soft,#222);font-variant-numeric:tabular-nums;">' + (r.volume / 1e6).toFixed(0) + '</td>' +
          '<td style="text-align:right;padding:.35rem .6rem;border-bottom:1px solid var(--border-soft,#222);font-variant-numeric:tabular-nums;">' + (r.denial != null ? (r.denial * 100).toFixed(1) + '%' : '—') + '</td>' +
          '<td style="text-align:right;padding:.35rem .6rem;border-bottom:1px solid var(--border-soft,#222);font-variant-numeric:tabular-nums;">' + r.mf.toLocaleString() + '</td>' +
        '</tr>';
      });
      html += '</tbody></table>';
      var wrap = document.getElementById('hmdaCountyTableWrap');
    ...
    function yoyDelta(curr, prev, priorYr) {
      if (prev == null || prev === 0) return 'vs ' + priorYr + ': —';
      var pct = ((curr - prev) / prev) * 100;
      var arrow = pct > 0 ? '▲' : pct < 0 ? '▼' : '→';
      var color = pct > 0 ? 'var(--good,#39c47e)' : pct < 0 ? 'var(--bad,#e05252)' : 'var(--muted)';
      return arrow + ' ' + Math.abs(pct).toFixed(1) + '% YoY';
    }
    function yoyDeltaPct(curr, prev, priorYr) {
      // For rates (already percentages 0-1), show the absolute pp change
      if (curr == null || prev == null) return 'vs ' + priorYr + ': —';
      var pp = (curr - prev) * 100;
      var arrow = pp > 0 ? '▲' : pp < 0 ? '▼' : '→';
      return arrow + ' ' + Math.abs(pp).toFixed(1) + 'pp YoY';
    }
    function yoyDeltaCurrency(curr, prev, priorYr) {
      if (prev == null || prev === 0 || curr == null) return 'vs ' + priorYr + ': —';
      var pct = ((curr - prev) / prev) * 100;
      var arrow = pct > 0 ? '▲' : pct < 0 ? '▼' : '→';
      return arrow + ' ' + Math.abs(pct).toFixed(1) + '% YoY';
    }
