  ...
  async function loadKalshiData() {
    try {
      ...
      try { json = await res.json(); } catch (_) {
        // Unavailable: response is not valid JSON (corrupted or empty file).
        return null;
      }
      // Unavailable: parsed object is falsy or the feed reported an error.
      if (!json || json.error) return null;
      // Unavailable: items array is absent or empty (workflow ran but produced no data).
      return (Array.isArray(json.items) && json.items.length > 0) ? json : null;
    } catch (_) {
      // Unavailable: network-level failure (offline, CORS, etc.).
      return null;
    }
  }
