...
(function () {
  ...
  /**
   * Is a Census API key configured for this browser?
   *
   * The published site ships none on purpose — js/config.js says so: "API keys
   * are intentionally empty here; data is served from cached JSON files
   * generated by GitHub Actions." A developer can still supply one through the
   * Data Quality Dashboard, which stores it in localStorage.
   *
   * This matters because a keyless call is not a call that might fail. Census
   * answers it with a 302 to missing_key.html carrying no
   * Access-Control-Allow-Origin header, so the browser reports it as a CORS
   * block — an error class fetch() cannot distinguish from a flaky network, and
   * one that retrying can never clear.
   */
  function _censusKeyConfigured() {
    // window.HNAUtils.censusKey() is the one reader of APP_CONFIG.CENSUS_API_KEY
    // that the other eight call sites already use; reading the config directly
    // here would make two producers of one answer that could drift apart.
    const key = (window.HNAUtils && window.HNAUtils.censusKey && window.HNAUtils.censusKey()) || '';
  ...
  async function _fetchCensusUrl(url, contextLabel) {
    ...
    // Without a key the request is not merely likely to fail, it is certain to
    // — so spending retries and vintage fallbacks on it buys nothing and costs
    // the visitor the whole wait. Measured on cohoanalytics.com 2026-09-19,
    // place 0828745: fetchWithTimeout retries 3x with 1s/2s backoff, the
    // vintage walk repeats that for 2024/2023/2022/2021, and the result was 30
    // doomed requests over 17.3 seconds and 207 console errors before the page
    // fell through to the cached JSON. Every number a visitor sees today
    // already comes from that fallback, because this function already returns
    // null on every published page load; returning null now is the same answer
    // sooner.
    if (!_censusKeyConfigured()) {
      if (window.HNAUtils.DEBUG_HNA) {
        console.info('[HNA] ' + label + ' skipped: no Census API key configured; using cached data.');
      }
      return null;
    }
