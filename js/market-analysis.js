...
(function () {
  ...
  var WEIGHTS = PMAScoring.WEIGHTS;

  // Risk thresholds (per PMA_SCORING.md)
  var RISK = PMAScoring.RISK;
  ...
  function scoreDemand(acs) {
    ...
    return PMAScoring.scoreDemand(acs);
  }
  ...
  function scoreCaptureRisk(acs, existingUnits, proposedUnits, chasEligible) {
    ...
    return PMAScoring.scoreCaptureRisk(acs, existingUnits, proposedUnits, chasEligible);
  }
  ...
  function scoreMarketTightness(acs) {
    ...
    return PMAScoring.scoreMarketTightness(acs);
  }
  ...
  function _scoreWorkforceWithCoverage(acs, lat, lon, bufTracts, countyAmi) {
    ...
    // ── EXCLUDED (#1560) ──
    // the synthetic fixture (deleted 2026-09-09) declares itself "Synthetic
    ...
    var cdleScore = null;
    reasons.push('CDLE vacancy (20%): excluded — no CDLE labor-market source is wired in');

    // ── 4. CDE school quality proximity (15%) ───────────────────────
    // ── EXCLUDED (#1560) ──
    // the synthetic fixture (deleted 2026-09-09) is synthetic by its own meta.note and
    ...
    var cdeScore = null;
    reasons.push('CDE school quality (15%): excluded — no CDE school-performance source is wired in');

    // ── 5. CDOT traffic connectivity (15%) ──────────────────────────
    // ── EXCLUDED (#1560) ──
    // the synthetic fixture (deleted 2026-09-09) is synthetic by its own meta.note, from
    // the same 2026-03-09 generation batch as the CDE and CDLE files.
    var cdotScore = null;
    reasons.push('CDOT connectivity (15%): excluded — no CDOT traffic source is wired in');
  ...
  function runAnalysis(lat, lon, options) {
    ...
    _scenarioScorer = function (units) {
      return computePma(acs, lihtcUnits, units, lat, lon, bufTracts, _pmaCountyAmi, nearbyLihtc, acsIdx);
    };
  ...
  function loadOverlays() {
    ...
    return Promise.all([
      ...
      var landOverlay = window.PublicLandOverlay;
      if (landOverlay && typeof landOverlay.load === 'function' && results[5]) {
        landOverlay.load(results[5]);
      }
    }).catch(function (e) {
