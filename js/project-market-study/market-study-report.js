...
(function (root, factory) {
  ...
  var G2 = "CHAS cannot see above 100% HAMFI, so the 100–120% band's observed pool is structurally zero pending an above-100% income source.";
  ...
  var LEGEND = 'Classification legend: observed / derived / modeled / user_entered / not_available';
  ...
  var INTERNAL_CAVEATS = Object.freeze([
    BANNER, 'hypothesis_to_test', 'owner_inputs_pending', G2, COMPETITIVE,
  ...
  function buildReport(model, meta) {
    ...
    var mixRows = scenario.program.unit_mix.map(function (row) {
      return '<tr><td>' + display(row.count) + '</td><td>' + display(row.bedrooms) + '</td><td>' + display(row.sqft_range[0]) + '–' + display(row.sqft_range[1]) + ' sq ft</td><td>' + badge(row.classification) + '</td></tr>';
    });
    var amiRows = scenario.program.ami_mix.map(function (row) {
      return '<tr><td>' + display(row.band[0], 'rate') + '–' + display(row.band[1], 'rate') + '</td><td>' + display(row.count) + '</td><td>' + badge(row.classification) + '</td></tr>';
    });
    var partnerRows = scenario.partners.map(function (row) {
      return '<tr><td>' + escape(row.role) + '</td><td>' + display(row.name || row.provider_id) + '</td><td>candidate — no commitment</td><td>' + badge(row.classification) + '</td></tr>';
    });
    var project = '<section><h2>1. Project summary</h2><p><strong>Jurisdiction:</strong> ' + escape(scenario.jurisdiction.name) + '</p><p><strong>Total units:</strong> ' + display(scenario.program.total_units.value) + ' ' + badge(scenario.program.total_units.classification) + '</p><p><strong>Tenure form:</strong> ' + display(scenario.program.tenure_form.value) + ' ' + badge(scenario.program.tenure_form.classification) + '</p><h3>Unit mix and sizes</h3>' + table(['Units', 'Bedrooms', 'Size', 'Classification'], mixRows) + '<h3>AMI mix</h3>' + table(['AMI band', 'Units', 'Classification'], amiRows) + '<h3>Partners</h3>' + table(['Role', 'Candidate', 'Status', 'Classification'], partnerRows) + '<p class="warning"><strong>FHA disambiguation:</strong> ' + FHA + '.</p></section>';

    var bandRows = model.derived.bands.map(function (row) {
      return '<tr><td>' + display(row.band[0], 'rate') + '–' + display(row.band[1], 'rate') + '</td><td>' + display(row.count) + '</td><td>' + display(row.maxAffordablePrice, 'money') + '</td><td>' + display(row.gapVsLocalPrice, 'money') + '</td><td><strong>' + escape(row.assistanceRangeCheck) + '</strong> finding</td><td>' + badge(row.classification) + '</td></tr>';
    });
    var selectedOutcome = model.selectedConvention.results[model.selectedYear];
    var affordability = '<section><h2>2. Affordability &amp; gap</h2><p><strong>Local home value:</strong> ' + display(scenario.local_baseline.home_value.value, 'money') + ' — ' + escape(scenario.local_baseline.home_value.source) + ' (' + badge(scenario.local_baseline.home_value.classification) + ')</p>' + table(['AMI band', 'Units', 'Max price', 'Gap vs local price', 'Assistance-range finding', 'Classification'], bandRows) + '<p><strong>Income required at selected settlement:</strong> ' + display(selectedOutcome.futureBuyerIncomeNeeded, 'money') + ' ' + badge(selectedOutcome.classification) + '</p></section>';

    var costs = Object.keys(scenario.costs).map(function (key) {
      var item = scenario.costs[key];
      return '<tr><td>' + escape(key) + '</td><td>' + display(item.value, 'money') + '</td><td>' + badge(item.classification) + '</td></tr>';
    });
    var pending = scenario.meta.owner_inputs_pending.map(function (item) { return '<li>' + escape(item) + '</li>'; }).join('');
    var costSection = '<section><h2>3. Costs &amp; subsidy</h2>' + table(['Input', 'Value', 'Classification'], costs) + '<p><strong>TDC per unit:</strong> ' + display(model.derived.tdcDependent.tdcPerUnit, 'money') + '</p><p><strong>Subsidy per unit:</strong> ' + display(model.derived.tdcDependent.subsidyPerUnit, 'money') + '</p><h3>What this report is waiting on</h3><p><strong>owner_inputs_pending</strong></p><ul>' + pending + '</ul></section>';
    ...
    var landCards = model.landOutcomes.map(function (item) {
      var checks = Object.keys(item.row.assessments).map(function (key) {
        ...
        return '<li><strong>' + escape(key) + ':</strong> ' + display(field.value) + ' ' + badge(field.classification) + ' <span class="verify">VERIFY — ' + escape(field.validator) + '</span></li>';
      }).join('');
      return '<article><h3>' + escape(item.row.label) + '</h3><p>' + (item.row.modelId === 'model_a_public_land_retention' ? '<strong>hypothesis_to_test</strong>' : '') + '</p><p>Initial per-unit affordability benefit: ' + display(item.row.initialPerUnitAffordabilityBenefit, 'money') + '</p><p>Monthly housing cost at year 5: ' + display(item.lifecycle.results[5].monthlyHousingCost, 'money') + ' ' + badge(item.lifecycle.classification) + '</p><ul>' + checks + '</ul></article>';
    }).join('');
    ...
    var conventionRows = model.conventionResults.map(function (result) {
      ...
      return '<tr><td>' + escape(result.conventionLabel) + '</td><td>' + display(outcome.ownerNetProceeds, 'money') + '</td><td>' + escape(outcome.preservesAffordabilityLabel) + '</td><td>' + badge(result.classification) + (result.verifyParameter ? ' <span class="verify">VERIFY parameter</span>' : '') + '</td><td>' + escape(result.scenarioLabel) + '</td></tr>';
    });
    ...
    var equity = '<section><h2>5. Shared equity &amp; settlement</h2>' + table(['Convention', 'Owner outcome', 'Affordability outcome', 'Classification', 'Scenario'], conventionRows) + '<h3>Selected settlement</h3><p>' + escape(model.settlement.scenarioLabel) + ' ' + badge(model.settlement.classification) + '</p><p>Public subsidy retained in home: <strong>' + display(model.settlement.publicSubsidyRetainedInHome, 'money') + '</strong></p><p>Public subsidy recaptured at sale: <strong>' + display(model.settlement.publicSubsidyRecapturedAtSale, 'money') + '</strong></p><p>Owner net proceeds: <strong>' + display(model.settlement.ownerNetProceeds, 'money') + '</strong></p><p>' + TRANSPARENCY + '</p>' + warning + '</section>';

    var funnelRows = model.funnel.stages.map(function (stage) {
      return '<tr><td>' + escape(stage.id) + '</td><td>' + display(stage.share, 'rate') + '</td><td>' + display(stage.outputCount) + '</td><td>' + escape(stage.label || '') + '</td><td>' + escape(stage.basis) + '</td><td>' + badge(stage.classification) + '</td></tr>';
    });
    ...
    var legend = '<section><h2>9. Classification legend</h2><p><strong>' + LEGEND + '</strong></p><dl><dt>observed</dt><dd>Directly reported source value.</dd><dt>derived</dt><dd>Engine output from observed inputs.</dd><dt>modeled</dt><dd>Scenario result, not an observed outcome.</dd><dt>user_entered</dt><dd>Caller or project-owner assumption.</dd><dt>not_available</dt><dd>Required evidence or owner input is absent.</dd></dl><p><strong>Commitment-status rule:</strong> ' + COMMITMENT + '.</p><p>All time-shaped paths retain the suffix “' + SCENARIO + '.”</p></section>';
