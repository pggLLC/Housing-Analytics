...
(function (root, factory) {
  ...
  function buildReport(model, meta) {
    ...
    var project = '<section><h2>1. Project summary</h2>' + plain('the example project being screened: how many homes, what sizes, how many are priced for each income group, and who might build and run it. The project is an example; the market figures in section 2 are for the jurisdiction named here.') + '<p><strong>Jurisdiction:</strong> ' + escape(jurisdictionLabel) + '</p><p><strong>Total homes:</strong> ' + display(scenario.program.total_units.value) + ' ' + badge(scenario.program.total_units) + '</p><p><strong>Home type:</strong> ' + display(scenario.program.tenure_form.value) + ' ' + badge(scenario.program.tenure_form) + '</p><h3>Unit mix and sizes</h3>' + table(['Homes', 'Bedrooms', 'Size', 'Where the number comes from'], mixRows) + '<h3>Homes by income group</h3>' + table(['Income group (AMI band)', 'Homes', 'Where the number comes from'], amiRows) + '<h3>Partners</h3>' + table(['Role', 'Candidate', 'Status', 'Where the number comes from'], partnerRows) + '<p class="warning"><strong>Not the same agency:</strong> ' + FHA + '. The local housing authority is a possible land owner here; the federal mortgage-insurance agency is not involved.</p></section>';
    ...
    var costSection = '<section><h2>3. Costs &amp; subsidy</h2>' + plain('what it would cost to build each home, and how much public money would be needed to sell it at the prices in section 2. Total development cost (TDC) is everything it takes to build: land, construction, design and permits, fees and financing. None of these figures has been supplied for the example project yet, so this section lists what is still needed.') + table(['Cost', 'Amount', 'Where the number comes from'], costs) + '<p><strong>TDC per unit:</strong> ' + display(model.derived.tdcDependent.tdcPerUnit, 'money') + '</p><p><strong>Subsidy per unit:</strong> ' + display(model.derived.tdcDependent.subsidyPerUnit, 'money') + '</p><h3>What this report is waiting on</h3><p><strong>Values still needed</strong> from the project sponsor:</p><ul>' + pending + '</ul></section>';
