...
(function (root, factory) {
  ...
  function renderConventions(model) {
    ...
    return `<section id="ms-s3" class="chart-card ms-section">${heading('3. What happens when an owner sells', 'shared-equity resale formulas')}${plain('price-restricted homes come with a resale formula that caps what an owner can sell for, so the home stays affordable for the next buyer. Each card below is one common formula. Compare two things: what the seller walks away with, and whether the capped price is still within reach of the next buyer. The market path menu changes how fast home values are assumed to rise or fall.')}<label>Market path <select id="ms-path-select">${pathOptions}</select></label><span class="ms-control-note">Each market path is a scenario, not a prediction.</span><div class="ms-card-grid">${cards}</div></section>`;
  }
  ...
  function renderSettlement(model) {
    ...
    return `<section id="ms-s4" class="chart-card ms-section">${heading('4. Where the money goes at resale', 'resale settlement')}${plain('pick a resale formula and a year to follow one sale line by line: selling costs are paid first, then the mortgage, then the owner gets their down payment back, then any public help is repaid. Whatever is left goes to the owner. A shortfall means there was not enough money to pay that line in full.')}<div class="ms-controls"><label>Resale formula <select id="ms-convention-select">${conventionOptions}</select></label><label>Year <select id="ms-year-select">${yearOptions}</select></label></div><p>${model.settlement.scenarioLabel} ${pill(model.settlement)}</p>${table(['Step', 'Owed', 'Paid', 'Shortfall', 'Evidence'], rows, 'Resale settlement steps')}<div class="ms-grid"><p>Public subsidy retained in home: <strong>${display(model.settlement.publicSubsidyRetainedInHome, 'money')}</strong></p><p>Public subsidy recaptured at sale: <strong>${display(model.settlement.publicSubsidyRecapturedAtSale, 'money')}</strong></p><p>Owner net proceeds: <strong>${display(model.settlement.ownerNetProceeds, 'money')}</strong></p></div>${warning}</section>`;
  }
  ...
  function renderFunnel(model) {
    ...
    return `<section id="ms-s5" class="chart-card ms-section">${heading('5. How many local households could buy', 'effective-demand funnel')}${plain('this starts with every local renter household in the project\'s income range, then narrows it down, step by step, to the households who could and would actually buy one of these homes. What is left at the bottom is the realistic buyer pool.')}${howTo}<p class="ms-caveat">Nothing you type is saved — reloading the page clears it.</p><p><strong>Steps still blank:</strong> ${model.funnel.unresolvedStages.length ? model.funnel.unresolvedStages.map(stageLabel).join('; ') : 'none'}</p>${table(['Stage', 'Share / output (decimal share, e.g. 0.8 = 80%)', 'Output / protected label', 'Evidence basis', 'Classification'], rows, 'Effective-demand funnel')}</section>`;
  }
  ...
  function renderCapture(model) {
    ...
    return `<section id="ms-s6" class="chart-card ms-section">${heading('6. How fast the homes might sell', 'capture scenarios')}${plain('given the realistic buyer pool from section 5, this asks what share of those buyers the project would have to sign up to sell every home in two, two and a half, three or four years. The bigger the share, the harder the sales job. It stays blank until section 5 is complete.')}${table(['Sales pace', 'Homes closing each month', 'Homes closing each year', 'Share of ready buyers needed each year', 'Share of all ready buyers the project needs', 'Signed contracts needed (some fall through)', 'Counts buyers used up by earlier sales'], scenarioRows, 'Capture scenarios')}${table(['Income group (AMI band)', 'Homes in this group', 'Share of this group\'s ready buyers needed', 'Data limitation'], amiRows, 'AMI capture cross-tab')}<div class="ms-warning"><strong>Other homes for sale are not counted:</strong> ${model.capture.competitiveSupplyNote}</div><div class="ms-warning"><strong>Treat these as rough arithmetic:</strong> ${model.capture.captureHumilityCaveat}</div></section>`;
  }
