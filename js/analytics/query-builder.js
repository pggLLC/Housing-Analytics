...
(function () {
   ...
   */
  function QueryBuilder(container, options) {
  ...
  function applyFilter(row, filter) {
    var dim = DIMENSIONS[filter.dimension];
    if (!dim) return true;
    var field = dim.field;
    var rawVal = row[field];
    if (rawVal === null || rawVal === undefined) return false;
    var val = coerce(rawVal, dim.type);
    var op  = filter.operator;

    switch (op) {
      case 'eq':
        return val === coerce(filter.values[0], dim.type);
      case 'neq':
        return val !== coerce(filter.values[0], dim.type);
      case 'gt':
        return val > coerce(filter.values[0], dim.type);
      ...
      case 'lte':
        return val <= coerce(filter.values[0], dim.type);
      case 'between': {
        var lo = coerce(filter.values[0], dim.type);
        var hi = coerce(filter.values[1], dim.type);
        return val >= lo && val <= hi;
      }
      case 'in_list': {
        var coercedList = (filter.values || []).map(function (v) {
          return coerce(v, dim.type);
        });
  ...
  // Also expose pure helpers for testing without DOM
  window.QueryBuilder._applyFilter   = applyFilter;
  window.QueryBuilder._DIMENSIONS    = DIMENSIONS;
  window.QueryBuilder._OPERATORS     = OPERATORS;
