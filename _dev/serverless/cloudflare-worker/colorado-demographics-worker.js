/**
 * Cloudflare Worker: Colorado Demographics
 *
 * Endpoint: GET /co-demographics
 *
 * Fetches data from the Colorado State Demography Office (SDO) APIs:
 * - Population forecasts by county
 * - Housing unit forecasts
 * - Migration data by region
 *
...
/**
 * Attempt to fetch population forecast data from the CO SDO GIS API.
 * Returns an array of county forecast objects, or throws on failure.
 */
async function fetchPopulationForecast() {
  ...
  const res = await fetch(url, {
    headers: { "Accept": "application/json", "User-Agent": "co-demographics-worker" },
    cf: { cacheTtl: 0 }  // bypass CF cache for the sub-request; we cache the whole response
  });
  if (!res.ok) throw new Error(`CO SDO population API ${res.status}`);
  const raw = await res.json();

  // SDO returns an array of { county, fips, year, population } rows; reshape to nested structure
...
/**
 * Attempt to fetch housing unit forecast data from the CO SDO GIS API.
 */
...
/**
 * Attempt to fetch migration data from the CO SDO GIS API.
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname !== "/co-demographics") {
    ...
    try {
      // Attempt to fetch live data from CO SDO APIs (all three in parallel)
      ...
      // Only accept the live data if we got non-empty arrays
      ...
      // Fall back to embedded data so the endpoint always returns something useful
